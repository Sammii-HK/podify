...
describe("device-keys", () => {
  beforeEach(async () => {
    ...
    delete process.env.PODIFY_DAILY_DEVICE_QUOTA;
  });
  ...
  it("createDeviceKey returns a record with a dk_ prefixed key", async () => {
    ...
    expect(record.usageToday).toHaveLength(0);
  });
  ...
  it("checkAndConsumeQuota allows generations under the default quota (3/day)", async () => {
  ...
  it("checkAndConsumeQuota rejects the 4th generation once the daily quota is hit", async () => {
    const { createDeviceKey, checkAndConsumeQuota } = await import(
      "@/lib/device-keys"
    );
    const record = await createDeviceKey("device-abc");
  ...
  it("respects PODIFY_DAILY_DEVICE_QUOTA env override", async () => {
    process.env.PODIFY_DAILY_DEVICE_QUOTA = "1";
    const { createDeviceKey, checkAndConsumeQuota, DAILY_QUOTA } =
  ...
  it("different devices have independent quotas", async () => {
    ...
    await checkAndConsumeQuota(deviceA.key);
    await checkAndConsumeQuota(deviceA.key);
    await checkAndConsumeQuota(deviceA.key);
